...
static uint32_t                             m_packet_id        = 1;                                 /**< MQTT packet number. */
...
static void app_mqtt_evt_handler(mqtt_client_t * p_client, const mqtt_evt_t * p_evt)
{
    ...
    switch(p_evt->id)
    {
        ...
        {
            APPL_LOG ("[APPL]: >> MQTT_EVT_PUBLISH\r\n");
            ...
            memcpy(m_data_rcv,
                   p_evt->param.pub_message.message.payload.p_bin_str,
                   p_evt->param.pub_message.message.payload.bin_strlen);
            
            m_data_rcv[p_evt->param.pub_message.message.payload.bin_strlen] = 0;
            ...
            APPL_LOG ("[APPL]: Current value of channel Temperature: %s\r\n", p_data_start);
            
            if (p_evt->param.pub_message.message.topic.qos == MQTT_QoS_0_AT_MOST_ONCE)
            {
                // Send pub ack.
                err_code = mqtt_publish_ack(p_client, p_evt->param.pub_message.message_id);
                if(err_code != NRF_SUCCESS)
                {
                    APPL_LOG ("[APPL]: mqtt_publish_ack failed, reason 0x%08x\r\n", err_code);
                }
            }
...
static void app_xively_subscribe(void)
{
    ...
    // Create MQTT topic.
    #define NUMBER_OF_SUBSCRIBED_TOPICS  1
    
    mqtt_topic_t topic[NUMBER_OF_SUBSCRIBED_TOPICS];
    
    topic[0].topic.p_utf_str   = (uint8_t *)APP_MQTT_XIVELY_API_URL;
    topic[0].topic.utf_strlen  = strlen(APP_MQTT_XIVELY_API_URL);
    topic[0].qos               = MQTT_QoS_0_AT_MOST_ONCE;
    
    mqtt_subscription_list_t subscription_list;
    
    subscription_list.list_count = NUMBER_OF_SUBSCRIBED_TOPICS;
    subscription_list.p_list     = &topic[0];
    subscription_list.message_id = m_packet_id;
    ...
    {
        m_packet_id+= 2; // Avoid ever sending invalid message id 0.
    }
