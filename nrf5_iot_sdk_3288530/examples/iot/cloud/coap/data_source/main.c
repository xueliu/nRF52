...
/** @file
 *
 * @defgroup iot_coap_exosite_data_source main.c
 * @{
...
#define COAP_POSTING_TOGGLE_BUTTON    BSP_BUTTON_0                                                  /**< Button used to start/stop periodic CoAP requests. */
...
#define COAP_POST_REQ_INTERVAL_MS     2300                                                          /**< CoAP request interval. */
...
static int                    m_data_points[NUMBER_OF_DATA_SAMPLES] = \
                                                           { 0,  8,  15,  19,  20,  19,  15,  8, \
                                                             0, -8, -15, -19, -20, -19, -15, -8};   /**< Data samples being sent repeatedly. */
static uint8_t                m_sample_index = 0;                                                   /**< Index of data sample being sent. */
static bool                   m_is_ipv6_if_up = false;                                              /**< Variable representing the state of the IPv6 interface. */
static bool                   m_do_coap_requests = false;                                           /**< Variable representing whether periodic CoAP POST requests are sent. */
...
static const char             m_uri_part_alias[] = "nordicsrc01";                                   /**< Value of a CoAP Uri-Path option. See Exosite CoAP API doc for details. */       
...
static uint16_t               m_global_token_count = 0x0102;                                        /**< Token value of CoAP messages. */
...
{
    APPL_LOG("[APPL]: Sending CoAP POST request, with payload %4d... \r\n", m_data_points[m_sample_index]);
    ...
    sprintf(payload, "%4d", m_data_points[m_sample_index]);
    err_code = coap_message_payload_set(p_request, payload, sizeof(payload)-1);
    APP_ERROR_CHECK(err_code);

    m_sample_index += 1;
    if (m_sample_index == NUMBER_OF_DATA_SAMPLES)
    {
        m_sample_index = 0;
    }
...
static void coap_request_commence(iot_timer_time_in_ms_t wall_clock_value)
{
    UNUSED_PARAMETER(wall_clock_value);
    ...
    // Check if CoAP requests are enabled.
    if (m_do_coap_requests == false)
    {
        return;
    }
    ...
    if ((m_app_state == APP_STATE_COAP_REQUEST_IN_PROGRESS) || \
...
static void button_event_handler(uint8_t pin_no, uint8_t button_action)
{
    ...
    {
        if (pin_no == COAP_POSTING_TOGGLE_BUTTON)
        {
            if (m_do_coap_requests == false)
            {
                m_do_coap_requests = true;
                APPL_LOG("\r\n[APPL]: Periodic CoAP POST requests enabled. \r\n\r\n");
            }
            else if (m_do_coap_requests == true)
            {
                m_do_coap_requests = false;
                APPL_LOG("\r\n[APPL]: Periodic CoAP POST requests disabled. \r\n\r\n");
            }
        }
    }
}
...
static void buttons_init(void)
{
    ...
    {
        {COAP_POSTING_TOGGLE_BUTTON, false, BUTTON_PULL, button_event_handler},
#ifdef COMMISSIONING_ENABLED
...
void ip_app_handler(iot_interface_t * p_interface, ipv6_event_t * p_event)
{
    ...
    switch(p_event->event_id)
    {
        case IPV6_EVT_INTERFACE_ADD:
            ...
            // New connection: force DNS resolution.
            memset(m_coap_server_address.u8, 0x00, IPV6_ADDR_SIZE);
...
static void iot_timer_init(void)
{
    ...
    static const iot_timer_client_t list_of_clients[] =
    {
        ...
        {coap_request_commence,   COAP_POST_REQ_INTERVAL_MS},
#ifdef COMMISSIONING_ENABLED
