...
uint32_t nrf_mem_reserve(uint8_t ** pp_buffer, uint32_t * p_size)
{
    ...
    {
        MM_LOG ("[MM]: Memory reservation result 0x%08lx, size 0x%08lx!",
                err_code,
                (*p_size));
...
void * nrf_calloc(uint32_t count, uint32_t size)
{
    ...
    MM_LOG ("[nrf_calloc]: Requested size 0x%08lx, count 0x%08lx\r\n", allocated_size, count);

    uint32_t retval = nrf_mem_reserve(&buffer,&allocated_size);
    if (retval == NRF_SUCCESS)
    {
        MM_LOG ("[nrf_calloc]: buffer %p, total size 0x%08lx\r\n", buffer, allocated_size);
        memset(buffer,0, allocated_size);
    }
    else
    {
        MM_LOG("[nrf_calloc]: Failed to allocate memory 0x%08lx\r\n", allocated_size);
        buffer = NULL;
    }
...
void nrf_free(void * p_mem)
{
    ...
    for (index = 0; index < TOTAL_BLOCK_COUNT; index++)
    {
        if (&m_memory[memory_index] == p_mem)
        {
            ...
            MM_LOG("[MM]: << Freeing block 0x%08lx.\r\n", index);
            block_init(index);
            break;
        }
